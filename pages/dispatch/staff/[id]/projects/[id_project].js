...
const Project = () => {
  ...
  useEffect(() => {
    setProject(Array.isArray(p.dispatchProjects) ? p.dispatchProjects[0] : '')
  }, [p.isLoading])
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch(`/api/dispatch_projects/${router.query.id_project}`, {method: method, body:JSON.stringify(project)});
    !project.id && router.push(`/dispatch/staff/${router.query.id}/projects`)
  }
  ...
  async function handleDelete(e) {
    e.preventDefault()
    if (confirm('削除します')) {
      const res = await fetch(`/api/dispatch_projects/${router.query.id_project}`, {method: 'DELETE', body:JSON.stringify(project)});
      router.push(`/dispatch/staff/${router.query.id}/projects`)
    }
  }
        ...
        <form onSubmit={handleSubmit}>
          ...
          期間
            <div className="flex items-center mb-2">
              ...
              <div className="w-2/3">
                <input type="date" name="periodStart" value={project.periodStart || ''} onChange={handleChange}
                      className="form-inline-input"/>
              </div>
            </div>
            ...
            <div className="flex items-center mb-2">
              ...
              <div className="w-2/3">
                <input type="date" name="periodEnd" value={project.periodEnd || ''} onChange={handleChange}
                      className="form-inline-input"/>
              </div>
            </div>
          </div>
