...
export function useCompany(id = '') {
  const { data, error } = useSWR(`/api/dispatch_companies/${id}`, fetcher)
  const isLoading = !error && !data
  const isError = error
...
export function useContract({id = '', dispatchedStaffId = ''} = {}) {
  ...
  const { data, error } = useSWR(`/api/dispatch_contracts/${id}${param}`, fetcher)
  const isLoading = !error && !data
  const isError = error
  const hasContracts = Boolean(data?.dispatchContracts)

  return {
    dispatchContracts: data?.dispatchContracts || null,
    isLoading: isLoading,
    isError: isError
  }
}
...
export function useDispatchProject({id = '', dispatchedStaffId = ''} = {}) {
  ...
  const isLoading = !error && !data
  const isError = error
  const hasDispatchProjects = Boolean(data?.dispatchProjects)

  return {
    dispatchProjects: data?.dispatchProjects || null,
    isLoading: isLoading,
    isError: isError
  }
}

export function useDispatchWorkingStatus({id = '', dispatchedStaffId = ''} = {}) {
  const param = dispatchedStaffId ? `?dispatchedStaffId=${dispatchedStaffId}` : ''
  const { data, error } = useSWR(`/api/dispatch_working_status/${id}${param}`, fetcher)
  const isLoading = !error && !data
  const isError = error
  const hasDispatchWorkingStatus = Boolean(data?.dispatchWorkingStatus)

  return {
    dispatchWorkingStatus: data?.dispatchWorkingStatus || null,
    isLoading: isLoading,
    isError: isError
  }
}

export function useDispatchPersonInCharge({id = '', companyId = ''} = {}) {
  const param = companyId ? `?companyId=${companyId}` : ''
  console.log('param = ' + param)
  const { data, error } = useSWR(`/api/dispatch_person_in_charge/${id}${param}`, fetcher)
  console.log('`/api/dispatch_person_in_charge/${id}${param}` = ' + `/api/dispatch_person_in_charge/${id}${param}`)
  const isLoading = !error && !data
  const isError = error
  const hasPersonInCharge = Boolean(data?.personInCharge)
...
export function useDispatchCompanyContracts({id = '', companyId = ''} = {}) {
  const param = companyId ? `?companyId=${companyId}` : ''
  const { data, error } = useSWR(`/api/dispatch_company_contracts/${id}${param}`, fetcher)
  const isLoading = !error && !data
  const isError = error
  const hasDispatchCompanyContracts = Boolean(data?.companyContracts)

  return {
    companyContracts: data?.companyContracts || null,
    isLoading: isLoading,
    isError: isError
  }
}