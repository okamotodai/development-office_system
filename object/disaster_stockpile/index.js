...
const DisasterStockpiles = () => {
  ...
  const changeColor = (limitday, term, now) => {
    ...
    const before = limit.setDate(limit.getDate() - term);//期限から7日前
    const remainingTime = (now - before) / 86400000;//今日（ミリ秒）から期限から7日前（ミリ秒）を引く
...
const tbody = !disasterStockpile.disaster_stockpile ? null : disasterStockpile.disaster_stockpile.map(d =>
  <Link href="/object/disaster_stockpile/[id_disaster_stockpile]" as={`/object/disaster_stockpile/${d.id}`} key={d.id}>
    <tr className="hover:bg-gray-200" style={changeColor(d.storagePeriod,d.alert,now) === true ? { background:"#ffe4e1" }:{}}>
     ...
     <table className="block overflow-x-scroll whitespace-no-wrap">
       <thead>
         <tr>
           ...
           <th className="text-left p-2">期限前アラート期間</th>
         </tr>
       </thead>
